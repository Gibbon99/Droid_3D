...
//-----------------------------------------------------------------------------
{
	...
	con_print ( true, true, "Number of functions in script [ %i ]", numFunctionsInScripts );
...
//-----------------------------------------------------------------------------
{
	...
	if ( functionName.size() < 1 )
		{
			con_print ( true, false, "Error: Failed to provide function name." );
			return false;
		}
	...
	if ( ret != asEXECUTION_FINISHED )
		{
			...
			if ( ret == asEXECUTION_ABORTED )
			...
			else if ( ret == asEXECUTION_EXCEPTION )
				{
					...
					con_print ( true, true, "Line: [ %i ]", scriptContext->GetExceptionLineNumber() );
					con_print ( true, true, "Desc: [ %s ]", scriptContext->GetExceptionString() );
